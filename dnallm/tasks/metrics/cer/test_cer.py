...
class TestCER(unittest.TestCase):
    def test_cer_case_sensitive(self):
        ...
        assert abs(char_error_rate - 0.1818181818 < 1e-6)
    ...
    def test_cer_whitespace(self):
        ...
        char_error_rate = cer.compute(predictions=preds, references=refs)
        assert abs(char_error_rate - 0.1111111 < 1e-6)

        refs = ["werewolf"]
        preds = ["weae     wolf"]
        # S = 1, D = 1, I = 0, N = 8, CER = 0.25
        char_error_rate = cer.compute(predictions=preds, references=refs)
        assert abs(char_error_rate - 0.25 < 1e-6)
        ...
        char_error_rate = cer.compute(predictions=preds, references=refs)
        assert abs(char_error_rate - 0.0 < 1e-6)
    ...
    def test_cer_sub(self):
        ...
        char_error_rate = cer.compute(predictions=preds, references=refs)
        assert abs(char_error_rate - 0.125 < 1e-6)
    ...
    def test_cer_list_of_seqs(self):
        ...
        char_error_rate = cer.compute(predictions=preds, references=refs)
        assert abs(char_error_rate - 0.03571428 < 1e-6)
    ...
    def test_correlated_sentences(self):
        ...
        assert abs(char_error_rate - 0.071428 < 1e-6)
    ...
    def test_cer_unicode(self):
        ...
        char_error_rate = cer.compute(predictions=preds, references=refs)
        assert abs(char_error_rate - 0.4545454545 < 1e-6)

        refs = ["我能吞下玻璃", "而不伤身体"]
        preds = ["我    能 吞 下 玻 璃", "而不伤身体"]
        # S = 0, D = 5, I = 0, N = 11, CER = 5 / 11
        char_error_rate = cer.compute(predictions=preds, references=refs)
        assert abs(char_error_rate - 0.454545454545 < 1e-6)

        refs = ["我能吞下玻璃而不伤身体"]
        char_error_rate = cer.compute(predictions=refs, references=refs)
        assert not char_error_rate, "Expected char_error_rate to be 0.0"
