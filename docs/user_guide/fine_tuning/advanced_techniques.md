...
If the model support memory-efficient attention mechanisms, such as flash-attn.
Enable it in the model `config.json` file.
    ...
    def __iter__(self):
        ...
        for idx in self.indices:
            ...
            if (
                ...
                or current_tokens + sequence_length > self.max_tokens_per_batch
            ):
                if batch:
                    yield batch
                    batch = []
